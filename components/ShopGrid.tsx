...
export default function ShopGrid({ onQuickView }: { onQuickView: (p: Product) => void }) {
  ...
  return (
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
      {products.map((p) => (
        <article key={p.slug} className="spot-card group relative rounded-[20px]">
          <a href={`/product/${p.slug}`} className="relative block overflow-hidden rounded-t-[20px]">
          ...
          <div className="p-5">
            ...
            <a href={`/product/${p.slug}`} className="mt-1 block font-serif text-2xl hover:text-deepgold">{p.name} →</a>
